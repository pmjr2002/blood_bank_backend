...
@router.get('/', response_model=list[schemas_.DonorOut])
def get_donors_by_address_and_blood_group(locality:str = '',blood_group: str = '',db: Session = Depends(get_db)):
    ...
    if locality == '' and blood_group == '':
        db_donors = db.query(models_.Donor).all()
    elif locality == '':
        db_donors = db.query(models_.Donor).filter(models_.Donor.blood_group == blood_group).all()
    elif blood_group == '':
        address = locality + ', Mysuru'
        db_donors = db.query(models_.Donor).filter(models_.Donor.address == address).all()
    else:
        address = locality + ', Mysuru'
        db_donors = db.query(models_.Donor).filter(models_.Donor.address == address, models_.Donor.blood_group == blood_group).all()
...
def create_donor(request: schemas_.DonorBase, db: Session = Depends(get_db)):
    db_donor = db.query(models_.Donor).filter(models_.Donor.name == request.name).first()
...
def delete_donor(donor_id: str, db: Session = Depends(get_db)):
    db_donor = db.query(models_.Donor).filter(models_.Donor.donor_id == donor_id).first()
    if not db_donor:
